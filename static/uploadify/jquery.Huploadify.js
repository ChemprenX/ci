...
$.fn.Huploadify = function(opts){
	...
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><span class="uploadbtn">上传</span></div>';
	var defaults = {
		...
		fileSizeLimit:29233,//允许上传的文件大小，单位KB
				  ...
				  for(var i=0,len=files.length;i<len;i++){
				  	var thisFile = files[i];
				  		if(parseInt(formatFileSize(thisFile.size,true))>option.fileSizeLimit){
				  			alert('文件'+thisFile.name+'大小超出限制！'+parseInt(formatFileSize(thisFile.size,true)));
				  			continue;
				  		}
